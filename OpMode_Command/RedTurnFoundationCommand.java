...
public class RedTurnFoundationCommand extends BaseCommand {
    private FoundationHauler foundationHaluer;
    private Claw claw;
    private Double desiredOrientationAngle;

    public RedTurnFoundationCommand(LinearOpMode opMode, HkRobot robot, Double desiredOrientationAngle) {
        super(opMode, robot);
    ...
    public void run() {
        ...
        MoveUntilObjectCommand moveUntilWallCommand = new MoveUntilObjectCommand(opMode, robot, robot.getBackDistanceSensor(), 24.5, -0.7, false, 0.0, 10,
                -0.15, 39, -0.2, false, desiredOrientationAngle, 0.04);
        moveUntilWallCommand.run();
        sleepWithActiveCheck(100);

        drivetrain.rotateClockwise(0.2, 0.6, 85);
        claw.releaseStone();
        ...
        drivetrain.strafeLeft(0.3, 8, false, 0, true);
